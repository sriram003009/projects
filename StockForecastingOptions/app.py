...
# Main panel
# --------------------------------------------------------------------------- #
st.title("Options Contract — Last 10 Trading Sessions")
st.caption(
    "Enter a ticker, option type, expiration (MM/DD) and strike on the left, "
    "then click **Fetch Data**. Data is sourced from Yahoo Finance via yfinance."
)
...
# --------------------------------------------------------------------------- #
# 9. Forecasts (next 5 trading days)
# --------------------------------------------------------------------------- #
st.markdown("---")
st.markdown("## Forecasts — Next 5 Trading Days")
st.caption(
...
st.dataframe(
    ...
    column_config={
        ...
        "Day-N option (median)": st.column_config.NumberColumn(
            f"Day-{forecast_days}", format="$%.2f"
        ),
        "Lower band": st.column_config.NumberColumn("Lower", format="$%.2f"),
        "Upper band": st.column_config.NumberColumn("Upper", format="$%.2f"),
    },
)
...
with tab_a:
    st.markdown("**Monte Carlo + Black-Scholes** — 5,000 GBM paths revalued via BS each day.")
    ...
    fig_a.add_trace(go.Scatter(
        x=future_dates, y=mc_df["o_p90"], name="P90", mode="lines",
        line=dict(color="rgba(38,166,154,0.0)"),
        showlegend=False,
    ))
    fig_a.add_trace(go.Scatter(
        x=future_dates, y=mc_df["o_p10"], name="P10–P90 band", mode="lines",
        fill="tonexty", fillcolor="rgba(38,166,154,0.18)",
        line=dict(color="rgba(38,166,154,0.0)"),
    ))
    fig_a.add_trace(go.Scatter(
        x=future_dates, y=mc_df["o_p50"], name="Median forecast",
        mode="lines+markers", line=dict(color="#26a69a", width=2, dash="dash"),
    ))
    fig_a.update_layout(
        height=420, hovermode="x unified",
        title="Option price — historical & forecast cone",
        margin=dict(l=10, r=10, t=40, b=10),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "u_p10": st.column_config.NumberColumn("Underlying P10", format="$%.2f"),
            "u_p50": st.column_config.NumberColumn("Underlying P50", format="$%.2f"),
            "u_p90": st.column_config.NumberColumn("Underlying P90", format="$%.2f"),
            "o_p10": st.column_config.NumberColumn("Option P10", format="$%.2f"),
            "o_p50": st.column_config.NumberColumn("Option P50", format="$%.2f"),
            "o_p90": st.column_config.NumberColumn("Option P90", format="$%.2f"),
        },
    )
...
with tab_c:
    ...
    if arima_df is None:
        ...
        fig_c.add_trace(go.Scatter(
            x=future_dates, y=arima_df["o_high"], name="Upper", mode="lines",
            line=dict(color="rgba(255,167,38,0.0)"), showlegend=False,
        ))
        fig_c.add_trace(go.Scatter(
            x=future_dates, y=arima_df["o_low"], name="80% CI", mode="lines",
            fill="tonexty", fillcolor="rgba(255,167,38,0.18)",
            line=dict(color="rgba(255,167,38,0.0)"),
        ))
        fig_c.add_trace(go.Scatter(
            x=future_dates, y=arima_df["o_mean"], name="ARIMA mean",
        ...
        st.dataframe(
            ...
            column_config={
                "day": st.column_config.NumberColumn("Day"),
                "u_mean": st.column_config.NumberColumn("Underlying mean", format="$%.2f"),
                "u_low": st.column_config.NumberColumn("Underlying low", format="$%.2f"),
                "u_high": st.column_config.NumberColumn("Underlying high", format="$%.2f"),
                "o_mean": st.column_config.NumberColumn("Option mean", format="$%.2f"),
                "o_low": st.column_config.NumberColumn("Option low", format="$%.2f"),
                "o_high": st.column_config.NumberColumn("Option high", format="$%.2f"),
            },
        )
...
with tab_d:
    ...
    if rf_df is None:
        ...
        st.dataframe(
            ...
            column_config={
                "day": st.column_config.NumberColumn("Day"),
                "u_predicted": st.column_config.NumberColumn("Underlying", format="$%.2f"),
                "o_predicted": st.column_config.NumberColumn("Option", format="$%.2f"),
            },
        )

# --------------------------------------------------------------------------- #
# 10. What-If Scenario (Greeks-based + BS reprice)
# --------------------------------------------------------------------------- #
st.markdown("---")
st.markdown("## What-If Scenario — Greeks & Reprice")
st.caption(
...
gc = st.columns(5)
gc[0].metric("Delta", f"{g['delta']:.4f}",
             help="Δ option price per $1 underlying move")
gc[1].metric("Gamma", f"{g['gamma']:.4f}",
             help="Δ Delta per $1 underlying move (convexity)")
gc[2].metric("Theta", f"${g['theta_per_day']:.4f}/day",
             help="$ option price loss per calendar day, holding S and σ fixed")
gc[3].metric("Vega", f"${g['vega_per_pct']:.4f}/%",
             help="$ option price change per +1 percentage point of IV")
gc[4].metric("Rho", f"${g['rho_per_pct']:.4f}/%",
             help="$ option price change per +1 percentage point of risk-free rate")
...
sc1.metric("Current option price", f"${scn['current_price']:.2f}")
sc2.metric(
    "Greeks-based estimate",
    f"${scn['greeks_estimate']:.2f}",
    delta=f"{scn['greeks_estimate'] - scn['current_price']:+.2f}",
    help="Current + Δ·ΔS + ½·Γ·ΔS² + Θ·Δt + Vega·Δσ",
)
sc3.metric(
    "Black-Scholes reprice",
    f"${scn['bs_reprice']:.2f}",
    delta=f"{scn['bs_reprice'] - scn['current_price']:+.2f}",
    help="Full BS price at the target underlying, target time-to-expiry, and "
         "scenario IV. This is the source-of-truth value Nasdaq-style risk "
         "systems use; the Greek attribution explains it.",
)
diff = scn["greeks_estimate"] - scn["bs_reprice"]
sc4.metric(
    "Greeks − BS (residual)",
    f"${diff:+.2f}",
    help="Equal to −residual. Captures higher-order curvature (Speed, Color), "
         "vol convexity (Vomma), and cross-terms (Vanna, Charm) not in the "
         "2nd-order Taylor expansion. Large gaps mean the move is big enough "
         "that those higher-order Greeks matter.",
)

# --- P&L attribution waterfall (Nasdaq-style risk explain) --------------- #
st.markdown("### P&L attribution — Nasdaq-style explain")
st.caption(
    "Decomposes the **full Black-Scholes reprice ΔP** into Greek contributions. "
    "The residual captures higher-order curvature (Speed/Color), vol convexity "
    "(Vomma), and cross-terms (Vanna/Charm) that aren't in the 2nd-order Taylor "
    "expansion. The bars sum exactly to the BS reprice ΔP."
)

waterfall_components = [
    ("Current price", scn["current_price"], "absolute"),
    ("Delta · ΔS", scn["delta_pnl"], "relative"),
    ("½ · Gamma · ΔS²", scn["gamma_pnl"], "relative"),
    ("Theta · Δt", scn["theta_pnl"], "relative"),
    ("Vega · Δσ", scn["vega_pnl"], "relative"),
    ("Residual (cross-terms, higher-order)", scn["residual"], "relative"),
    ("BS reprice", scn["bs_reprice"], "total"),
]
